...
class CaretBurstDetector:
    """Detect rapid caret event bursts that indicate streaming output.

    During AI CLI responses, cargo builds, and similar streaming output,
    caret events fire faster than human navigation speed. This detector
    identifies such bursts and signals callers to suppress per-character
    speech, avoiding a flood of extraneous announcements.
    ...
    Usage::
        ...
        def event_caret(self, obj, nextHandler):
            ...
            if detector.should_speak():
                self._announceCursorPosition(obj)

    Args:
        burst_threshold_ms: Maximum gap (ms) between events to count
            as part of a burst. Default 150.
        burst_count: Minimum number of rapid events to trigger burst
            suppression. Default 3.
    """

    def __init__(self, burst_threshold_ms=150, burst_count=3):
        self._threshold_s = burst_threshold_ms / 1000.0
        self._burst_count = burst_count
        self._timestamps = []
        self._in_burst = False

    def record_event(self, timestamp):
        """Record a caret event timestamp.

        Call this with ``time.monotonic()`` on each caret event.
        Trims old timestamps outside the detection window.
        """
        # Trim timestamps older than the window
        cutoff = timestamp - self._threshold_s * self._burst_count
        self._timestamps = [
            t for t in self._timestamps if t > cutoff
        ]
        self._timestamps.append(timestamp)

        # Check if recent events form a burst
        if len(self._timestamps) >= self._burst_count:
            # Look at the last burst_count events
            recent = self._timestamps[-self._burst_count:]
            span = recent[-1] - recent[0]
            max_span = self._threshold_s * (self._burst_count - 1)
            self._in_burst = span <= max_span
        else:
            self._in_burst = False

    def should_speak(self):
        """Return True if the current event should produce speech.

        Returns False during a detected burst (streaming output).
        """
        return not self._in_burst

    def reset(self):
        """Clear burst state. Called on focus change or mode switch."""
